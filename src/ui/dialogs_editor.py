...
def _apply_changes(state: dict, project_id: str) -> bool:
    ...
    if current_db_state and current_db_state.get("status") in [ProjectStatus.STARTING.value, ProjectStatus.GENERATING.value]:
        st.error("Operation aborted: The project is actively generating in another session.")
        return False
...
def render_editor(navigate_to: Callable[[str, Optional[str]], None]) -> None:
    ...
    if "editor_state" not in st.session_state or st.session_state.editor_state.get("id") != project_id:
        db = st.session_state.db
    ...
    is_locked = state.get("status") in [ProjectStatus.STARTING.value, ProjectStatus.GENERATING.value]
    ...
    if line_count <= MAX_DIALOG_LINES:
        edit_mode = st.radio("Editing Mode", ["Visual Editor", "JSON Editor"], horizontal=True, disabled=is_locked)
    else:
        st.info(f"Visual Editor disabled for massive projects ({line_count} lines). Limit is {MAX_DIALOG_LINES}.")
    ...
    if edit_mode == "JSON Editor":
        with st.container(border=True):
            ...
            up_file = st.file_uploader("Upload Script JSON", type=['json'], disabled=is_locked)
            if up_file and not is_locked:
                try:
                    new_data = json.load(up_file)
                    new_def = new_data["definition"] if "definition" in new_data else new_data
        ...
        with st.container(border=True):
            ...
            with c_meta[0]:
                state["definition"]["name"] = st.text_input("Project Name", value=state["definition"].get("name", ""), disabled=is_locked)
            with c_meta[1]:
                state["definition"]["default_language"] = st.selectbox("Base Lang", options=lang_options, index=lang_options.index(state["definition"].get("default_language", "es")), disabled=is_locked)
            with c_meta[2]:
                tags_val = ", ".join(state["definition"].get("tags", []))
                p_tags_raw = st.text_input("Tags", value=tags_val, disabled=is_locked)
                state["definition"]["tags"] = [t.strip() for t in p_tags_raw.split(",")] if p_tags_raw else []
            
            state["definition"]["description"] = st.text_area("Description", value=state["definition"].get("description") or "", height=68, disabled=is_locked)
        ...
        for i, line in enumerate(state["definition"].get("script", [])):
            with st.container(border=True):
                ...
                with row[0]:
                    line["voice_id"] = st.selectbox("Voice", options=list(voice_map.keys()), format_func=lambda x: voice_map.get(x, x), index=list(voice_map.keys()).index(line["voice_id"]) if line["voice_id"] in voice_map else 0, key=f"v_{i}", disabled=is_locked)
                with row[1]:
                    line["language"] = st.selectbox("Lang", options=lang_options, index=lang_options.index(line.get("language", state["definition"].get("default_language", "es"))), key=f"l_{i}", disabled=is_locked)
                with row[2]:
                    emo_opts = {"": "None (Neutral)"}
                    for eid, edata in emotions_cat.items():
                        loc = edata.get("localized_names", {}).get(line.get("language", "en"), edata.get("localized_names", {}).get("en", eid))
                        emo_opts[eid] = loc
                    line["emotion"] = st.selectbox("Emotion", options=list(emo_opts.keys()), format_func=lambda x: emo_opts[x], index=list(emo_opts.keys()).index(line.get("emotion")) if line.get("emotion") in emo_opts else 0, key=f"e_{i}", disabled=is_locked)
                with row[3]:
                    int_opts = {"": "Default"}
                    for iid, idata in intensities_cat.items():
                        loc_i = idata.get("localized_names", {}).get(line.get("language", "en"), idata.get("localized_names", {}).get("en", iid))
                        int_opts[iid] = loc_i
                    has_emo = bool(line.get("emotion"))
                    line["intensity"] = st.selectbox("Intensity", options=list(int_opts.keys()), format_func=lambda x: int_opts[x], index=list(int_opts.keys()).index(line.get("intensity")) if has_emo and line.get("intensity") in int_opts else 0, key=f"i_{i}", disabled=not has_emo or is_locked)
                with row[4]:
