...
def render_monitor(navigate_to: Callable[[str, Optional[str]], None]) -> None:
    ...
    renders comprehensive metadata. It implements a conditional polling architecture 
    that automatically refreshes the interface utilizing the global application interval 
    while the generation process is active, ceasing immediately upon completion or failure.
    ...
    header_cols = st.columns([1, 11])
    with header_cols[0]:
        if st.button("🏠", help="Back to Dashboard"):
            navigate_to("dashboard", None)
    with header_cols[1]:
        st.title(f"📊 Monitor: {project.definition.name}")
    ...
    with st.container(border=True):
        ...
        st.write(f"**Description:** {project.definition.description if project.definition.description else 'No description provided.'}")

    st.subheader("Global Controls & Export")
    ctrl_1, ctrl_2, ctrl_3, ctrl_4, ctrl_5, ctrl_6, ctrl_7 = st.columns(7)
    ...
    with ctrl_4:
        ...
        st.download_button(label="📄 JSON", data=script_json, file_name=f"{project.definition.name.replace(' ', '_')}_script.json", mime="application/json", use_container_width=True)
    with ctrl_5:
        if is_completed:
            ...
            st.download_button(label="📦 ZIP", data=zip_buffer.getvalue(), file_name=f"{project.definition.name.replace(' ', '_')}_audio.zip", mime="application/zip", use_container_width=True)
        else:
            st.button("📦 ZIP", use_container_width=True, disabled=True)
            
    with ctrl_6:
        if st.button("✏️ Edit", use_container_width=True, disabled=is_active):
    ...
    with r1_c1:
        search_q = st.text_input("Fuzzy Search", placeholder="Search text content...").lower()
    with r1_c2:
    ...
    for line in project.definition.script:
        ...
        m_search = not search_q or search_q in line.text.lower()
